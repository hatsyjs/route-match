...
describe('rmatchSearchParam', () => {
  ...
  describe('?param=value', () => {
    ...
    it('matches URL if one of required parameter values matches', () => {
      expect(
        routeMatch(urlRoute(new URL('http://localhost/?param=wrong&param=value')), pattern),
      ).toBeTruthy();
    });
    it('does not match URL without required parameter', () => {
      expect(routeMatch(urlRoute(new URL('http://localhost/?other=value')), pattern)).toBeNull();
  ...
  describe('<name>?param=value', () => {
    ...
    it('matches URL with required parameter with required value', () => {
      expect(
        routeMatch(urlRoute(new URL('http://localhost/path?param=value')), pattern),
      ).toBeTruthy();
    });
    it('does not match match URL with wrong path', () => {
      expect(
        routeMatch(urlRoute(new URL('http://localhost/wrong?param=value')), pattern),
      ).toBeNull();
    });
    it('matches URL with if one of required parameter values matches', () => {
      expect(
        routeMatch(urlRoute(new URL('http://localhost/path?param=wrong&param=value')), pattern),
      ).toBeTruthy();
    });
    it('does not match URL without required parameter', () => {
      expect(
        routeMatch(urlRoute(new URL('http://localhost/path?other=value')), pattern),
      ).toBeNull();
    });
    it('does not match URL if required parameter has wrong value', () => {
      expect(
        routeMatch(urlRoute(new URL('http://localhost/path?param=wrong')), pattern),
      ).toBeNull();
    });
  });
});
